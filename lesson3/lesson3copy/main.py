...
def get_data(url):
    ...
    for item in range(1, 24):
        ...
        for project_url in projects_urls:
            ...
            try:
                project_logo = "http://www.edutainme.ru" + project_data.find("div", class_="Ima logo").find("img").get("src")
            except Exception:
            ...
            projects_data_list.append(
                {
                    ...
                    "Url логотип проекта": project_logo,
                    "Короткое описания проекта": project_short_description,
                    "Сайт проект": project_website,
                    "Полное описание проекта": project_full_description.strip()
                }
            )

        iteration_count -= 1
        print(f"Итерация #{item} завершена, осталось итерация #{iteration_count}")
